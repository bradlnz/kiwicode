...
func newInteractiveTerminal(root, shell string, rows, cols int) (*interactiveTerminal, error) {
	...
	cmd := exec.Command(shell, "-i") // Bash reads ~/.bashrc; zsh/fish read their interactive configuration.
	cmd.Dir, cmd.Stdin, cmd.Stdout, cmd.Stderr = root, tty, tty, tty
...
func (e *editor) terminalEvents() <-chan terminalEvent {
	if e.switching != nil || e.shell.terminal == nil {
		return nil
	}
	return e.shell.terminal.events
}

func (e *editor) receiveTerminal(event terminalEvent, ok bool) bool {
	t := e.shell.terminal
	if !ok {
		t.events = nil
		t.ended = true
	...
	if len(event.data) > 0 {
		...
		if err := t.send(t.output()); err != nil {
			e.status = err.Error()
		}
	} else {
		t.ended = true
		t.events = nil
		e.status = "Shell exited; Ctrl+T to close, Ctrl+T again to restart"
		if event.err != nil {
			e.status += ": " + event.err.Error()
		}
	}
	return e.shell.open && e.shell.interactive
}

func (e *editor) openTerminal() {
	s := &e.shell
	s.interactive = true
	if s.terminal != nil && !s.terminal.ended {
		return
	}
	s.terminal.close()
	var err error
	_, height := e.panelHeights()
	s.terminal, err = newInteractiveTerminal(mustCwd(), os.Getenv("SHELL"), max(1, height-1), max(1, e.cols-e.sidebarWidth()))
	if err != nil {
		s.interactive = false
		e.status = "Terminal: " + err.Error()
		return
	}
	e.status = "Interactive shell · Ctrl+T returns to editor · wheel scrolls history"
}

func (e *editor) terminalFocused() bool {
	return e.shell.open && e.shell.focused && e.shell.interactive && e.shell.terminal != nil && e.popup == nil && !e.folderPrompt && !e.newFilePrompt && e.searchMode == ""
}

func (e *editor) handleTerminalMouse(k key) bool {
	if e.popup != nil || e.folderPrompt || e.newFilePrompt || e.searchMode != "" {
		return false
	}
	content, height := e.panelHeights()
	if height > 0 && k.x > e.sidebarWidth() && k.y >= 3+content && k.y < e.rows {
		if k.button == 0 && !k.release {
			e.shell.focused = true
		}
		if k.y > 3+content && e.shell.interactive && e.shell.terminal != nil {
			if err := e.shell.terminal.mouse(k); err != nil {
				e.status = err.Error()
			}
		} else if k.button == 64 || k.button == 65 {
			e.handleWheel(k)
		}
		return true
	}
...
func (t *interactiveTerminal) handle(k key) error {
	...
	keys := [...]C.VTermKey{C.VTERM_KEY_ESCAPE, C.VTERM_KEY_UP, C.VTERM_KEY_DOWN, C.VTERM_KEY_LEFT, C.VTERM_KEY_RIGHT, C.VTERM_KEY_HOME, C.VTERM_KEY_END, C.VTERM_KEY_PAGEUP, C.VTERM_KEY_PAGEDOWN, C.VTERM_KEY_DEL, C.VTERM_KEY_ENTER, C.VTERM_KEY_BACKSPACE, C.VTERM_KEY_TAB}
	if k.r != 0 {
		C.vterm_keyboard_unichar(t.native.vt, C.uint32_t(k.r), mod)
	} else if k.code < len(keys) {
		C.vterm_keyboard_key(t.native.vt, keys[k.code], mod)
	}
	return t.send(t.output())
}

func (t *interactiveTerminal) paste(text string) error {
	t.scroll = 0
	C.vterm_keyboard_start_paste(t.native.vt)
	data := append(t.output(), []byte(text)...)
	C.vterm_keyboard_end_paste(t.native.vt)
	return t.send(append(data, t.output()...))
}
...
func (t *interactiveTerminal) mouse(k key) error {
	...
	C.vterm_mouse_move(t.native.vt, C.int(k.y-t.y), C.int(k.x-t.x), C.VTERM_MOD_NONE)
	if k.button&32 == 0 {
		button := (k.button & 3) + 1
		if k.button&64 != 0 {
			button = (k.button & 1) + 4
